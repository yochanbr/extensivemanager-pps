...
function downloadPayslipPrintable() {
    const p = window.currentPayslip;
    if (!p) return showToast('No payslip loaded', 'error');

    const isNew = !!p.earnings;
    const basic = isNew ? p.earnings.basic : (p.basicSalary || 0);
    const gross = isNew ? basic + (p.earnings.allowances?.hra||0) + (p.earnings.allowances?.overtime||0) + (p.earnings.allowances?.other||0) : basic;
    const totalDed = isNew ? (p.deductions?.lop||0) + (p.deductions?.esi||0) + (p.deductions?.other||0) : (p.lopAmount||0);
    const net = gross - totalDed;

    const monthLabel = (p.month || '');
    const html = `
    <html><head><title>Payslip ${monthLabel}</title>
    <style>body{font-family:Inter, system-ui, Arial; padding:24px; color:#0f172a} .box{max-width:720px;margin:0 auto} .row{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px dashed #e6eef6} h1{font-size:20px}</style>
    </head><body><div class="box"><h1>Payslip - ${monthLabel}</h1>
    <div style="margin-top:12px"><strong>Employee:</strong> ${appState.employee.name || ''}</div>
    <div style="margin-top:12px"><div class="row"><span>Gross</span><span>₹${gross.toLocaleString()}</span></div>
    <div class="row"><span>Deductions</span><span>-₹${totalDed.toLocaleString()}</span></div>
    <div class="row"><strong>Net Pay</strong><strong>₹${net.toLocaleString()}</strong></div></div>
    <div style="margin-top:20px; font-size:12px; color:#64748b">Generated by Namma Mart</div>
    </div>
